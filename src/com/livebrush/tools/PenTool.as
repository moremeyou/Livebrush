package com.livebrush.tools
{
	...
	public class PenTool extends Tool
	{
		...
		{
			var pt:Point = Canvas.MOUSE_POINT//SyncPoint.localToLocal(Canvas.MOUSE_POINT, Canvas.WIREFRAME, Canvas.SELECTION);
			if (usePrevious)
			{
				if (copyFromRealLine)
				{
				...
				else 
				{
					...
					var dy:Number = pt.y-lastNewEdge.c.y;
					var dx:Number = pt.x-lastNewEdge.c.x;
			...
			{
				newEdge = new Edge(pt.x, pt.y, 25, 90, newLine.lines, 0xFF0000, newLine.weight, null);
			}
		...
		protected override function enterFrameHandler (e:Event):void
		{
			...
			if (updateNewEdge)
			{
				...
				if (newLine.length > 1)
				{
					...
					//Consol.Trace("PenTool: enterFrameHandler, IF 1");
					...
					newEdge.modify (pt, angleRads, newEdgeWidth); 
					
					if (LineLayer(activeLayer).line.length == 0) 
					//if (newLine.length == 2 && newLine is SmoothLine) 
					{
						//Consol.Trace("PenTool: enterFrameHandler, IF 2");
					...
					{
						//Consol.Trace("PenTool: enterFrameHandler, IF 3");
						
						newEdgeWidth = lastNewEdge.width;
						//var pt2:Point = SyncPoint.localToLocal(newLine.edges[newLine.edges.length-3].c, Canvas.WIREFRAME, Canvas.SELECTION);
						
						dy = pt.y-newLine.edges[newLine.edges.length-3].c.y;
						dx = pt.x-newLine.edges[newLine.edges.length-3].c.x;
				...
				{
					//Consol.Trace("PenTool: enterFrameHandler, IF 5");
					
					newEdgeWidth = newEdge.width;
					
					//// // Consol.Trace("loop");
					newEdge.modify (pt, Math.PI, newEdgeWidth);
				...
				if (newLine.length == 2 && LineLayer(activeLayer).line.length < 2) 
				{
					...
					//Consol.Trace("PenTool: enterFrameHandler, IF 6");
				...
				singleEdgeView.edge.modify (pt, angleRads, newEdgeWidth);
